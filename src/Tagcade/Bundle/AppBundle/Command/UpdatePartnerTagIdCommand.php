...
class UpdatePartnerTagIdCommand extends ContainerAwareCommand
{
    ...
    protected function configure()
    {
        $this
            ...
            ->addOption('id', 'i', InputOption::VALUE_OPTIONAL, 'id of a specific ad tag in the system.')
            ->addOption('ad-network', 'N', InputOption::VALUE_OPTIONAL, 'ad network id whose all ad tags\'s partner tag id will be updated')
            ->addOption('publisher', 'p', InputOption::VALUE_OPTIONAL, 'publisher id whose all ad tags\'s partner tag id will be updated')
            ->addOption('all', 'a', InputOption::VALUE_NONE, 'specify if all ad tags in the system need to be updated')
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $adNetworkId = filter_var($input->getOption('ad-network'), FILTER_VALIDATE_INT);
        ...
        if ($all === true) {
            ...
            if (!$adTag instanceof AdTagInterface) {
                $output->writeln('<error>either "--all" or "--publisher" or "--ad-network" or "--id" must be valid</error>');
                return false;
            }
