...
class AdTagVoter implements VoterInterface
{
    const VIEW = 'view';
    const EDIT = 'edit';
     ...
     */
    public function vote(TokenInterface $token, $adTag, array $attributes)
    {
        if (!$this->supportsClass(get_class($adTag))) {
        ...
        if ($user->hasRole('ROLE_PUBLISHER') && $user->getId() == $adTag->getAdSlot()->getSite()->getPublisherId()) {
            return VoterInterface::ACCESS_GRANTED;
        }
