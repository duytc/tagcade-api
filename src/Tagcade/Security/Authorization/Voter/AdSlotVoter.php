...
class AdSlotVoter implements VoterInterface
{
    const VIEW = 'view';
    const EDIT = 'edit';
     ...
     */
    public function vote(TokenInterface $token, $adSlot, array $attributes)
    {
        if (!$this->supportsClass(get_class($adSlot))) {
        ...
        if ($user->hasRole('ROLE_PUBLISHER') && $user->getId() == $adSlot->getSite()->getPublisherId()) {
            return VoterInterface::ACCESS_GRANTED;
        }
