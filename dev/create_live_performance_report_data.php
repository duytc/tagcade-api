...
foreach($testEventCounter->getAdSlotDataHourly() as $slotId => $slotDataHourly) {
    foreach ($slotDataHourly as $hour => $slotDataHourlyItem) {
        ...
        saveAdSlotData($cacheEventCounter, $slotId, $slotDataHourlyItem, $allAdSLotMap, $container, $testEventCounter);
    }
}
...
foreach($testEventCounter->getAdTagDataHourly() as $tagId => $tagDataHourly) {
    foreach ($tagDataHourly as $hour => $tagDataHourlyItem) {
        ...
        $dataWithDateHour->setTime($hour, 0);
        $cacheEventCounter->setDate($dataWithDateHour);
        saveAdTagData($cacheEventCounter, $tagId, $tagDataHourlyItem, $container, $testEventCounter);
    }
}
 ...
 */
function saveAdTagData (\Tagcade\Service\Report\PerformanceReport\Display\Counter\CacheEventCounter $cacheEventCounter, $tagId, $tagData, ContainerInterface $container, \Tagcade\Service\Report\PerformanceReport\Display\Counter\TestEventCounter $testEventCounter) {
    $cache = $container->get('tagcade.cache.app_cache');
 ...
 * @param \Tagcade\Service\Report\PerformanceReport\Display\Counter\TestEventCounter $testEventCounter
 */
function saveAdSlotData (\Tagcade\Service\Report\PerformanceReport\Display\Counter\CacheEventCounter $cacheEventCounter, $slotId, $slotData, $allAdSLotMap, ContainerInterface $container, \Tagcade\Service\Report\PerformanceReport\Display\Counter\TestEventCounter $testEventCounter) {
    $cache = $container->get('tagcade.cache.app_cache');
    $adSlot = findAdSlot($allAdSLotMap, $slotId);

//    $namespace = $cacheEventCounter->getNamespace($cacheEventCounter::NAMESPACE_AD_SLOT, $slotId);
//
//    $cacheKey = $cacheEventCounter->getCacheKey(
